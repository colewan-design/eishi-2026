...
.nav-bar.scrolled-bar {
  ...
  box-shadow: 0 1px 0 var(--linen) !important;
...
.nav-link:hover { color: var(--bronze-l); }
.nav-link.dark:hover { color: var(--bronze); }
.nav-link.router-link-active { color: var(--bronze-l); }
.nav-link.dark.router-link-active { color: var(--bronze); }
...
.lang-pill.dark {
  border-color: rgba(44,43,38,0.25);
  color: var(--ink);
}
.lang-pill:hover {
...
.desktop-nav-row.compact { padding: 16px 6vw; }
...
.hamburger-btn.dark .ham-line { background: var(--ink); }
.hamburger-btn.open .ham-line:nth-child(1) { transform: translateY(6px) rotate(45deg); }
.hamburger-btn.open .ham-line:nth-child(2) { opacity: 0; }
...
.close-btn:hover { border-color: var(--bronze-l); }
.close-x {
...
.drawer-links {
  display: flex; flex-direction: column;
  gap: 0; margin-top: 4rem; flex: 1;
}
.drawer-link {
  font-family: var(--fd);
  font-size: clamp(2.2rem, 8vw, 3.8rem);
  font-weight: 300; color: rgba(248,245,240,0.45);
  text-decoration: none;
  padding: 1.4rem 0;
...
.drawer-link:first-child { border-top: 1px solid rgba(255,255,255,0.07); }
.drawer-link:hover,
.drawer-link.router-link-active { color: var(--ivory); }
.drawer-link-arrow {
  font-size: 1rem; color: var(--bronze);
  opacity: 0; transition: opacity 0.3s, transform 0.3s;
}
.drawer-link:hover .drawer-link-arrow { opacity: 1; transform: translateX(4px); }
...
.dlang-btn.active { color: var(--bronze-l); }
.dlang-btn:hover  { color: var(--ivory); }
.dlang-sep {
  font-size: 0.65rem; color: rgba(255,255,255,0.15);
  align-self: center;
}
.drawer-tagline {
  font-family: var(--dm); font-size: 0.62rem;
...
<template>
  <v-app-bar
    ...
    class="nav-bar"
    :class="{ 'scrolled-bar': scrolled }"
    height="auto"
    style="background: transparent"
  >
    <!-- ── Desktop ────────────────────────────────── -->
    <div
      class="desktop-nav-row d-none d-md-flex"
      ...
      <div class="nav-right">
        <v-menu offset-y>
          <template #activator="{ props }">
            ...
            >
              <span class="lang-dot" />
              {{ selectedLanguage === 'Japanese' ? '日本語' : 'EN' }}
            </button>
          </template>
          <v-list density="compact" min-width="120">
            <v-list-item @click="changeLanguage('English')">
              <v-list-item-title style="font-size:0.85rem">English</v-list-item-title>
            </v-list-item>
            <v-list-item @click="changeLanguage('Japanese')">
              <v-list-item-title style="font-size:0.85rem">日本語</v-list-item-title>
            </v-list-item>
          </v-list>
        </v-menu>
      </div>
    ...
    <!-- ── Mobile bar ─────────────────────────────── -->
    <div class="mobile-nav-row d-flex d-md-none">
      <router-link to="/">
        <img
          src="/eishi/eishi_logo-removebg-preview.webp"
          alt="Eishi"
          class="mobile-logo"
          :style="{ filter: scrolled || isNotHome ? 'invert(1) brightness(0)' : 'none' }"
        />
      </router-link>

      <button
        class="hamburger-btn"
        :class="{ open: menuOpen, dark: scrolled || isNotHome }"
    ...
    <div class="mobile-drawer" :class="{ open: menuOpen }">
      ...
      <div class="drawer-top">
        <img src="/eishi/eishi_logo-removebg-preview.webp" alt="Eishi" class="drawer-logo" />
        <button class="close-btn" @click="menuOpen = false" aria-label="Close menu">
          <span class="close-x" />
        </button>
      </div>
      ...
      <div class="drawer-bottom">
        <div class="drawer-lang">
          ...
          >EN</button>
          <span class="dlang-sep">/</span>
          <button
            class="dlang-btn"
            :class="{ active: selectedLanguage === 'Japanese' }"
            @click="changeLanguage('Japanese')"
          >日本語</button>
        </div>
        <span class="drawer-tagline">Eishi Group</span>
      </div>
    </div>
  </v-app-bar>
</template>
